...
def lambda_handler(event, context):
    ...
    sns_body = json.loads(sqs_message['body'])  # Parse the SQS body
    print(f"SNS Parsed Body: {sns_body}")

    s3_event = json.loads(sns_body['Message'])  # Extract and parse the S3 event message
    print(f"S3 Parsed Event: {s3_event}")
